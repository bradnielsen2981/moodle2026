...
/**
 * Multi page course format related unit tests.
 *
 * @package    format_multipageformat
 * @copyright  2015 Marina Glancy
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
#[\PHPUnit\Framework\Attributes\CoversClass(\format_multipageformat::class)]
final class format_multipageformat_test extends \advanced_testcase {

    /**
     * Tests for format_multipageformat::get_section_name method with default section names.
     *
     * @return void
     */
    public function test_get_section_name(): void {
        ...
        $numsections = 5;
        $course = $generator->create_course(['numsections' => $numsections, 'format' => 'multipageformat'],
            ['createsections' => true]);
    ...
    /**
     * Tests for format_multipageformat::get_section_name method with modified section names.
     *
     * @return void
     */
    ...
    /**
     * Tests for format_multipageformat::get_default_section_name.
     *
     * @return void
     */
    public function test_get_default_section_name(): void {
        ...
        foreach ($coursesections as $section) {
            if ($section->section == 0) {
                $sectionname = get_string('section0name', 'format_multipageformat');
                $this->assertEquals($sectionname, $courseformat->get_default_section_name($section));
            } else {
                $sectionname = get_string('newsection', 'format_multipageformat');
                $this->assertEquals($sectionname, $courseformat->get_default_section_name($section));
            }
        }
    }
    ...
    public function test_update_inplace_editable(): void {
        ...
        $this->setUser($user);
        $course = $this->getDataGenerator()->create_course(['numsections' => 5, 'format' => 'multipageformat'],
            ['createsections' => true]);
        $section = $DB->get_record('course_sections', ['course' => $course->id, 'section' => 2]);

        // Call webservice without necessary permissions.
        try {
            \core_external::update_inplace_editable('format_multipageformat', 'sectionname', $section->id, 'New section name');
        ...
        $res = \core_external::update_inplace_editable('format_multipageformat', 'sectionname', $section->id, 'New section name');
    ...
    public function test_inplace_editable(): void {
        ...
        // Call callback format_multipageformat_inplace_editable() directly.
        $tmpl = component_callback('format_multipageformat', 'inplace_editable', ['sectionname', $section->id, 'Rename me again']);
    ...
    public function test_default_course_enddate(): void {
        ...
        $params = ['format' => 'multipageformat', 'numsections' => 5, 'startdate' => 1445644800];
    ...
    public function test_get_required_jsfiles(): void {
        ...
        $course = $generator->create_course(['format' => 'multipageformat']);
        $format = course_get_format($course);
