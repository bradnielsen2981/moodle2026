...
Feature: Multi page format supports indentation
  In order to create courses with a clear structure
  As a course creator
  I need my courses to support indentation for better organization

  @javascript
  Scenario: Admins could disable indentation in multi page format
    Given the following "courses" exist:
      | fullname | shortname | format          |
      | Course 1 | C1        | multipageformat |
    ...
    When the following config values are set as admin:
      | indentation | 0 | format_multipageformat |
    And I am on "Course 1" course homepage with editing mode on
    And I open "Test forum name" actions menu
