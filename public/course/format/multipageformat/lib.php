...
/**
 * This file contains main class for Multi page course format.
 *
 * @since     Moodle 2.0
...
/**
 * Main class for the Multi page course format.
 *
 * @package    format_multipageformat
 * @copyright  2012 Marina Glancy
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class format_multipageformat extends core_courseformat\base {
    ...
    public function uses_indentation(): bool {
        return (get_config('format_multipageformat', 'indentation')) ? true : false;
    }
    ...
    /**
     * Returns the default section name for the multi page course format.
     *
    ...
    public function get_default_section_name($section) {
        ...
        if ($section->sectionnum == 0) {
            return get_string('section0name', 'format_multipageformat');
        }

        return get_string('newsection', 'format_multipageformat');
    }
     ...
     *
     * Multi page format uses the following options:
     * - coursedisplay
     * - hiddensections
     *
    ...
    public function course_format_options($foreditform = false) {
        ...
        if ($foreditform && !isset($courseformatoptions['coursedisplay']['label'])) {
            ...
            $courseformatoptionsedit = [
                ...
                'hidegeneralsection' => [
                    'label' => new lang_string('hidegeneralsection', 'format_multipageformat'),
                    'element_type' => 'advcheckbox',
                    'help' => 'hidegeneralsection',
                    'help_component' => 'format_multipageformat',
                ],
            ];
     ...
     *
     * In case if course format was changed to 'multipageformat', we try to copy options
    ...
    /**
     * Returns whether a section is visible to the current user.
     *
    ...
    public function section_action($section, $action, $sr) {
        ...
        if ($section->section && ($action === 'setmarker' || $action === 'removemarker')) {
            // Format 'multipageformat' allows to set and remove markers in addition to common section actions.
        ...
        $renderer = $PAGE->get_renderer('format_multipageformat');
    ...
    public function get_config_for_external() {
        ...
        $formatoptions['indentation'] = get_config('format_multipageformat', 'indentation');
 ...
 */
function format_multipageformat_inplace_editable($itemtype, $itemid, $newvalue) {
    global $DB, $CFG;
    require_once($CFG->dirroot . '/course/lib.php');
    if ($itemtype === 'sectionname' || $itemtype === 'sectionnamenl') {
        $section = $DB->get_record_sql(
            'SELECT s.* FROM {course_sections} s JOIN {course} c ON s.course = c.id WHERE s.id = ? AND c.format = ?',
            [$itemid, 'multipageformat'], MUST_EXIST);
